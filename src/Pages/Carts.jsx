...
export const Carts = () => {
  ...
  const total = cart.reduce((sum, item) => sum + item.count * 100, 0);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-950 to-rose-950 px-4 sm:px-6 lg:px-12 py-8 text-white">
      ...
      <div className="mb-10">
        <h1 className="text-white text-xl">Cart Total: <span className="text-rose-400 font-bold">{total}$</span></h1>
      </div>
      <div>
        {activeCarts.length === 0 ? (
          <h1 className="text-xl text-center text-white">
            There is no food order.
          </h1>
        ) : (
